...
import numpy as np
#The default python installation for many engineers is the one which comes with ArcGIS.  
...
root = Tkinter.Tk()
root.withdraw()
fullpath = tkFileDialog.askopenfilename(parent = root, initialdir = 'c:' , 
...
flowTypes = ("*.u[0-9][0-9]","*.f[0-9][0-9]") #QuasiUnsteadyFlowFiles use a very different format and are not implemented, "*.q[0-9][0-9]")
flowFileList = []
for ft in flowTypes:
...
#GEOMETRY
#Loop through geom files (*.g## )
#Store the geometry Title and Description
...
print("Found " + str(len(flow_dict)) + " flow files.  " + str(len(flow_dict)-len(usedFlows )) +" are not used by any plans.")
print("Found " + str(len(geom_dict)) + " geometry files.  " + str(len(geom_dict)-len(usedGeom )) +" are not used by any plans.")
